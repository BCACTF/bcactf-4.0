...
void print_user(user_t* user) {
    printf("%.32s#%c%c%c%c", user->name,
        user->tag[0], user->tag[1], user->tag[2], user->tag[3]);
}
...
void print_msg(msg_t* msg) {
    printf("[");
    print_time(msg->time_sent);
    printf("] ");
    print_user(msg->author);
    printf(": ");
    for (size_t i = 0; msg->content[i] != '\0'; ++i) {
